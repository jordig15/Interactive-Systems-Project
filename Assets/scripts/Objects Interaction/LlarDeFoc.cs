...
public class LlarDeFoc : MonoBehaviour
{
    ...
    {
        firstTime = -1.0f;
        focActive = InteractionsManager.Instance.llarDeFoc1Active;
        Debug.Log("START" + focActive + InteractionsManager.Instance.llarDeFoc1Active);

        foc = GameObject.Find("fire");
    }

    // Update is called once per frame
    void Update()
    {
        //Debug.Log("LF" + focActive);
    ...
    void interact()
    {
        foc.active = false;
        focActive = true;
        InteractionsManager.Instance.llarDeFoc1Active = false;
    }
